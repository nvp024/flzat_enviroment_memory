"""Coordinate memory queries, clarification, and Nav2 commands."""
...
class MemoryCommandManager(Node):
    ...
    def __init__(self) -> None:
        ...
        self.create_subscription(
            String,
            "/environment_memory/text_command",
        ...
        self.get_logger().info(
            "Memory command manager ready; waiting for VAD audio events or "
            "text commands on /environment_memory/text_command."
        )
    ...
    def _on_text_command(self, message: String) -> None:
        with self._lock:
            if self._busy:
                self.get_logger().warn(
                    "Ignoring text while another command is active."
                )
                return
            self._busy = True
        try:
            self._handle_transcript(message.data)
        except Exception as exc:
    ...
    def _handle_transcript(self, transcript: str) -> None:
        ...
        if not transcript:
            raise CommandError("command transcript is empty")
    ...
    def _request_memory(self, intent: Intent, query_text: str) -> None:
        ...
        try:
            ...
            future.add_done_callback(
                lambda completed, requested_intent=intent,
                requested_query=query_text: self._on_query_result(
                    requested_intent, requested_query, completed
                )
            )
        except Exception as exc:
            self._speak_and_release(f"The memory query could not be sent. {exc}")

    def _on_query_result(self, intent: Intent, query_text: str, future) -> None:
        try:
            ...
            self._log_query_ranking(query_text, matches)
    ...
    def _log_query_ranking(
        self, query_text: str, matches: tuple[CommandMatch, ...]
    ) -> None:
        self.get_logger().info(
            f"Semantic Top-K: query={query_text!r}, results={len(matches)}"
        )
        if not matches:
            self.get_logger().info("Semantic Top-K: no matching objects")
            return
        for rank, match in enumerate(matches, start=1):
            self.get_logger().info(
                "Semantic Top-K: "
                f"rank={rank}, score={match.score:.4f}, label={match.label!r}, "
                f"scene={match.scene!r}, object_id={match.object_id!r}, "
                f"map=({match.x:.3f}, {match.y:.3f}, {match.z:.3f})"
            )
